...
export const sendMessageToGemini = async (
    ...
    try {
        ...
        // Standardize on gemini-3.5-flash for all requests. This is extremely capable, multimodal,
        // and fully supported on the free tier, avoiding any paid/billing 403 API key exceptions in production.
        const model = 'gemini-3.5-flash';
        
        // Real-time information detection for Google Search Grounding
        // General GST, Income Tax, ROC, Audit, Company Law, and Transfer Pricing questions are answered directly by Gemini.
        // Google Search is used only for genuinely live queries such as "latest notification", "recent circular", "today", "current amendment", or "breaking news".
        ...
        // Robust sequence sanitization to conform strictly to the Gemini Chat API specification:
        // 1. The sequence must start with a 'user' turn.
        // 2. Turns must alternate strictly between 'user' and 'model'.
        const contents: any[] = [];
        let expectedRole: 'user' | 'model' = 'user';

        for (const item of rawContents) {
            if (item.parts.length === 0) continue; // Skip empty parts
            ...
            if (item.role === expectedRole) {
                ...
                // If it is a leading model turn (e.g. welcome message), we MUST skip it
                // because the conversation must start with a 'user' turn.
                // Otherwise, merge consecutive model turns.
                const lastItem = contents[contents.length - 1];
