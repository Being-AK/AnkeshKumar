...
export default function RotatePdfTool({ onClose }: RotatePdfToolProps) {
  ...
  const handleFileChange = async (files: File[]) => {
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        // getRotation() returns an object like { angle: number, type: string } in modern pdf-lib
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] my-8 animate-scale-in">
        
        {/* Workspace Header */}
        <WorkspaceHeader
          title="Rotate PDF Workspace"
          subtitle="Local sandboxed rotation: Preserves 100% of original visual quality"
            ...
            <div className="space-y-6">
                ...
                leftSection={
                  <>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      <button
                        onClick={selectAll}
                        className="px-2.5 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 font-bold text-[10px] rounded-md hover:border-corporate dark:hover:border-gold transition-all cursor-pointer"
                      >
                        All
                      </button>
                      <button
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-extrabold text-navy dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  <span>Interactive Page Workspace</span>
                </h4>
                ...
                <div className="border border-slate-100 dark:border-slate-800/80 rounded-2xl p-6 bg-slate-50/10 dark:bg-slate-950/5 max-h-[35vh] overflow-y-auto">
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    {pagesState.map((page) => {
                      ...
                      const isRotated = page.currentRotation !== page.originalRotation;
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Rotated badge indicator */}
                          {isRotated && (
                            <div className="absolute top-2 right-2 z-10 bg-orange-500 text-white font-black text-[8px] px-1 py-0.5 rounded shadow">
                              {page.currentRotation}°
                            </div>
                          )}

                          {/* Page Artboard Preview Box */}
                          <div className="w-16 h-20 bg-slate-50 dark:bg-slate-900 border border-slate-150 dark:border-slate-800 rounded shadow-inner flex items-center justify-center relative mt-4 overflow-hidden">
                            {/* Document icon matching current rotation angle */}
                            <div 
                            ...
                            >
                              <FileText size={32} className="text-slate-350 dark:text-slate-600" />
                              {/* Page indicator inside the artboard */}
                              <span className="absolute text-[8px] font-black text-slate-500 dark:text-slate-450 select-none">
                                {page.pageIndex + 1}
                              </span>
                            </div>
                          </div>
