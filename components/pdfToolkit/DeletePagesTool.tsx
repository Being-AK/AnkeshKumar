                ...
                <div className="border border-slate-100 dark:border-slate-800/80 rounded-2xl p-6 bg-slate-50/10 dark:bg-slate-950/5 max-h-[35vh] overflow-y-auto">
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    {Array.from({ length: totalPages || 0 }).map((_, i) => {
                      const isDeleted = selectedPages.has(i);
                      ...
                      return (
                        <div
                          ...
                          className={`relative border rounded-xl p-3 flex flex-col items-center justify-between gap-3 cursor-pointer select-none transition-all duration-300 ${
                            isDeleted
                              ? 'border-rose-400 dark:border-rose-800 bg-rose-500/5 dark:bg-rose-950/10 ring-1 ring-rose-400/30 dark:ring-rose-800/30'
                        ...
                        >
                          ...
                          <div className="absolute top-2 left-2 z-10 text-slate-400 dark:text-slate-600">
                            {isDeleted ? (
                              <CheckSquare size={15} className="text-rose-500 dark:text-rose-400" />
                            ) : (
                              <Square size={15} />
                            )}
                          </div>
                          ...
                          {/* Delete overlay text */}
                          {isDeleted && (
                            <div className="absolute top-2 right-2 z-10 bg-rose-500 text-white font-black text-[8px] px-1 py-0.5 rounded shadow flex items-center gap-0.5">
                              <Trash2 size={8} />
                              <span>DELETE</span>
                            </div>
                          )}
                          ...
                          <div className={`w-16 h-20 border rounded shadow-inner flex items-center justify-center relative mt-4 overflow-hidden transition-all duration-300 ${
                            isDeleted 
                              ? 'bg-rose-100/30 dark:bg-rose-950/20 border-rose-200 dark:border-rose-900/40 opacity-60' 
                              : 'bg-slate-50 dark:bg-slate-900 border-slate-150 dark:border-slate-800'
                          }`}>
                            <div className="flex items-center justify-center">
                              <FileText size={32} className={isDeleted ? "text-rose-300 dark:text-rose-900/60" : "text-slate-350 dark:text-slate-600"} />
                              <span className={`absolute text-[10px] font-black select-none ${isDeleted ? 'text-rose-500 dark:text-rose-400 line-through' : 'text-slate-500 dark:text-slate-450'}`}>
                                {i + 1}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="text-center w-full">
                            <p className={`text-[10px] font-bold ${isDeleted ? 'text-rose-500 dark:text-rose-400 line-through' : 'text-slate-700 dark:text-slate-300'}`}>
                              Page {i + 1}
                            </p>
                            <p className="text-[8px] text-slate-400 dark:text-slate-500 mt-0.5">
                              {isDeleted ? 'Excluding' : 'Retaining'}
                            </p>
                          </div>
                        </div>
                      );
