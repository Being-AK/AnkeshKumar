...
export default function UnlockPdfTool({ onClose }: UnlockPdfToolProps) {
          ...
          prev.map((f) =>
            f.id === item.id
              ? { 
                  ...f, 
                  status: 'error', 
                  errorMsg: 'An unexpected error occurred while processing this document.',
                  errType: 'unexpected'
                }
              : f
          )
  ...
  const decryptSingleFile = async (
    ...
    try {
      const createModuleDefault = await import('@neslinesli93/qpdf-wasm');
      ...
      if (exitCode === 0 || exitCode === 3) {
        ...
        const decryptedBlob = new Blob([outputFile], { type: 'application/pdf' });
        
        // Clean up WASM virtual file system memory immediately
        try {
          qpdf.FS.unlink(inputPath);
          qpdf.FS.unlink(outputPath);
        } catch (cleanupErr) {
          console.warn('VFS cleanup error:', cleanupErr);
        }

        // Parse page count of decrypted PDF for perfect accuracy
        let pageCount: number | undefined;
        try {
          const { PDFDocument } = await import('pdf-lib');
          const pdfDoc = await PDFDocument.load(await decryptedBlob.arrayBuffer());
          pageCount = pdfDoc.getPageCount();
        } catch (err) {
          console.warn('Could not read page count of decrypted PDF:', err);
        }

        return { success: true, blob: decryptedBlob, pageCount };
      } else {
        // Clean up WASM virtual file system memory on failure too
        try {
          qpdf.FS.unlink(inputPath);
        } catch (cleanupErr) {}
        try {
        ...
        let errorMsg = 'An unexpected error occurred while processing this document.';

        const lowerStderr = stderrText.toLowerCase();
        if (lowerStderr.includes('password') || lowerStderr.includes('incorrect') || lowerStderr.includes('invalid') || lowerStderr.includes('wrong')) {
          errType = 'wrong_password';
          errorMsg = 'The password entered for this PDF is incorrect.';
          ...
          errorMsg = 'This document uses an encryption method that is not currently supported.';
      ...
      if (qpdf) {
        try {
          // Unlink any stray input/output files in the root folder during VM issues
      ...
      if (combinedError.includes('password') || combinedError.includes('incorrect') || combinedError.includes('invalid') || combinedError.includes('wrong')) {
        errType = 'wrong_password';
        errorMsg = 'The password entered for this PDF is incorrect.';
        console.warn('QPDF decryption processing validation: incorrect/missing password', errStr);
      } else if (combinedError.includes('unsupported') || combinedError.includes('algorithm') || combinedError.includes('crypt filter')) {
        errType = 'unsupported_encryption';
        errorMsg = 'This document uses an encryption method that is not currently supported.';
        console.warn('QPDF decryption processing validation: unsupported encryption', errStr);
        ...
        errType = 'corrupted';
        errorMsg = 'The PDF appears to be damaged or unreadable.';
        console.warn('QPDF decryption processing validation: file corrupt/invalid', errStr);
      } else {
        console.error('QPDF decryption processing error:', err);
      }
  ...
  const handleDecryptAll = async () => {
    ...
    for (const item of scannedFiles) {
      ...
      // Update current index and file name for dynamic progress display
      setCurrentFileIndex(currentIdx + 1);
      setCurrentFileName(item.name);

      // Update row status to decrypting
      setScannedFiles((prev) =>
  ...
  return (
    ...
    >
      <div 
        ...
        tabIndex={-1}
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-5xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] my-8 animate-scale-in relative outline-none"
      >
