...
export default function OcrPdfTool({ onClose }: OcrPdfToolProps) {
  ...
  // Lazy Load PDF.js from secure CDN
  const loadPdfJsLibrary = (): Promise<any> => {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => {
        setPdfLibLoading(false);
        reject(new Error('Failed to load PDF engine. Check your internet connection.'));
      };
      document.head.appendChild(script);
    });
  };

  // Lazy Load Tesseract.js from secure CDN
  const loadTesseractLibrary = (): Promise<any> => {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => {
        setOcrLibLoading(false);
        reject(new Error('Failed to load OCR engine. Check your internet connection.'));
      };
      document.head.appendChild(script);
    });
  };
  ...
  return (
    ...
    >
      <div 
        ...
        tabIndex={-1}
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-6xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] my-8 animate-scale-in outline-none"
      >
