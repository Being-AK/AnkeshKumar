...
export const ComplianceSuite: React.FC = () => {
  ...
  const previousToolIdRef = useRef<string>('gstin-search');

  // Smoothly scroll container to top whenever tool changes (but not on initial mount)
  useEffect(() => {
    if (previousToolIdRef.current !== currentToolId) {
  ...
  useEffect(() => {
    const handleGlobalSelect = (e: Event) => {
      ...
      if (['gstin-search', 'udyam-search', 'company-search', 'director-search', 'gstin-reg', 'udyam-reg', 'lei-reg', 'demat'].includes(id)) {
        ...
        setCurrentToolId(id);
      } else if (['calc-emi', 'calc-roi', 'calc-savings', 'calc-compound', 'calc-salary', 'calc-gst'].includes(id)) {
  ...
  const CALCULATOR_ITEMS = [
    { id: 'calc-gst', name: 'GST, Penalty & Cash Interest', icon: Calculator, short: 'Calculate GST splits, interest & delay fees' },
      ...
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <div className="text-center mb-16 relative">
          ...
          <h2 className="text-4xl font-extrabold text-navy dark:text-white tracking-tight">
            Ankesh Compliance Suite
          </h2>
          <p className="mt-4 text-sm text-slate-500 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed">
            Perform real-time direct GSTIN lookups, MCA company searches, and use our specialized financial calculators. 
            For any compliance queries, tax inquiries, or notice document analysis, use our <strong className="text-navy dark:text-white">Privacy-First AI Assistant</strong> floating in the bottom-right corner!
          </p>
            ...
            <div className="bg-white dark:bg-darkCard p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl relative min-h-[500px] flex flex-col justify-between overflow-hidden">
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800/80 flex flex-col sm:flex-row justify-between items-center gap-3">
                <div className="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                  ...
                  <span>Compliance Suite & Practice Tools Demo</span>
                </div>
                <a 
                ...
                >
                  📧 Connect with Ankesh <ArrowRight size={12} />
                </a>
              </div>
